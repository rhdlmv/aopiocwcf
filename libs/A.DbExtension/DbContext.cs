...
namespace A.DBExtension
{
    ...
    {
        private A.DBExtension.ExpressionConverter _expressionConverter;
        ...
        {
            return this.DbObjectOperator.ExecuteNonQuery(sql);
        }

        public DataTable ExecuteQuery(string sql)
        {
            return this.DbObjectOperator.ExecuteQuery(sql);
        }
        ...
        {
            return this.DbObjectOperator.ExecuteScalar<T>(sql);
        }
        ...
        {
            return (from field in typeof(T).GetFields(BindingFlags.NonPublic | BindingFlags.Instance)
        ...
        {
            string sql = string.Format("select count(*) from {0} where name = '{1}'", fullTableName, name);
            if (this.ExecuteScalar<int>(sql) == 0)
            {
                string str2 = string.Format("insert into {0}(id,name,value) values('{1}','{2}','{3}')", new object[] { fullTableName, Guid.NewGuid().ToString(), name, value });
        ...
        {
            return this.QueryCount<T>(this.ExpressionConverter.ToCondition(where, new object[0]));
        }
        ...
        {
            return this.QueryObject<T>(this.ExpressionConverter.ToCondition(where, new object[0]), checkExist);
        }
        ...
        {
            return this.QueryObjects<T>(this.ExpressionConverter.ToCondition(where, new object[0]), orderBy);
        }

        public List<T> QueryObjects<T>(string condition, params OrderBy[] orderBys) where T : DbObject, new()
        {
            return this.QueryObjects<T>(condition, 0x7fffffff, 0, orderBys);
        }

        public virtual List<T> QueryObjects<T>(Expression<Func<T, bool>> where, int rowCount, int pageIndex) where T : DbObject, new()
        {
            return this.QueryObjects<T>(this.ExpressionConverter.ToCondition(where, new object[0]), rowCount, pageIndex, new OrderBy[0]);
        }
        ...
        {
            return this.QueryObjects<T>(this.ExpressionConverter.ToCondition(where, new object[0]), rowCount, pageIndex, orderBys);
        }
        ...
        {
            return this.QueryObjects<F, T>(this.ExpressionConverter.ToCondition(where, new object[0]), mapper, rowCount, pageIndex, orderBys);
        }
        ...
        public DbObjectOperator DbObjectOperator { get; set; }

        public IDbRuleContext DbRuleContext { get; set; }

        private A.DBExtension.ExpressionConverter ExpressionConverter
        {
            get
            {
                if (this._expressionConverter == null)
                {
                    this._expressionConverter = new A.DBExtension.ExpressionConverter(this.DbObjectOperator.SqlFormatter);
                }
