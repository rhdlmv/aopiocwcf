...
namespace A.DBExtension
{
    public interface IRuleMapper<T> where T : DbObject, new()
    {
        ...
        IRuleMapper<T> PartialRuleFor<F>(Expression<Func<T, F>> field, Func<F, F> func, Func<T, bool> condition = null, DbOperation dbOperation = DbOperation.CU);
        IRuleMapper<T> PartialRuleFor<F1, F2>(Expression<Func<T, F1>> field, Expression<Func<T, F2>> compareField, Func<F1, F2, F1> compareFunc, Func<T, bool> condition = null, DbOperation dbOperation = DbOperation.CU);
        IRuleMapper<T> RemoveRule<F>(Expression<Func<T, F>> field);
        IRuleMapper<T> RuleFor<F>(Expression<Func<T, F>> field, Func<F, F> func, Func<T, bool> condition = null, DbOperation dbOperation = DbOperation.CU);
        IRuleMapper<T> RuleFor<F1, F2>(Expression<Func<T, F1>> field, Expression<Func<T, F2>> compareField, Func<F1, F2, F1> compareFunc, Func<T, bool> condition = null, DbOperation dbOperation = DbOperation.CU);
