namespace EgoalTech.DB.Extension
{
    ...
    public class DbRuleContext : IDbRuleContext
    {
        ...
        public IDbObjectInfo<T> GetDbObjectInfo<T>() where T: DbObject, new() => 
            this.GetRuleMapper<T>().GetDbObjectInfo()
        ...
        public IModelValidator<T> GetValidator<T>() where T: DbObject, new() => 
            this.GetRuleMapper<T>().GetModelValidator()
        ...
        private IRuleMapper<T> Init<T>(IRuleMapper<T> ruleMapper = null) where T: DbObject, new()
        {
            ...
            foreach (PropertyInfo info in type.GetProperties())
            {
                ...
                string fieldName;
                DynamicPropertyInfo info = DbObjectTools.GetDynamicPropertyInfo(type, info);
                if (info != null)
                {
                    ...
                    if (!ruleMapper.FieldRules.TryGetValue(info.Name, out list))
                    {
                        list = new List<FieldRule>();
                        ruleMapper.FieldRules.Add(info.Name, list);
                    }
                    getMethod = info.GetGetMethod();
                    fieldName = info.Name;
                    FieldRule rule2 = new FieldRule();
                    rule2.set_GetField(t => getMethod.Invoke(t, null));
                    rule2.set_Validate(delegate (object o, string action) {
                        ...
                        {
                            throw new FieldValidateException(exception.Message, fieldName, exception.get_FieldValue());
                        }
                        return obj2;
                    });
                    rule2.set_Actions(new string[] { DbOperation.Create.ToString(), (DbOperation.None | DbOperation.Update).ToString() });
                    if (func6 == null)
                    {
                        func6 = t => true;
                    }
                    rule2.set_When(func6);
