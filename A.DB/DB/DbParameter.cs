namespace EgoalTech.DB
{
    ...
    public class DbParameter
    {
        ...
        private string GetValue(string table, string name)
        {
            ...
            string sql = string.Format("select * from {0} where {1} = '{2}'", this.SqlFormatter.FormatTableOrViewName(table), str2, ConvertUtils.Escape(name));
        ...
        private void SetValue(string table, string name, string value)
        {
            ...
            string sql = string.Format("update {0} set {1} = '{2}' where {3} = '{4}'", new object[] { this.SqlFormatter.FormatTableOrViewName(table), str2, ConvertUtils.Escape(value), str, ConvertUtils.Escape(name) });
